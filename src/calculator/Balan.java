...
class Balan {
	...
	private boolean isOperator(String s) {
		String operator[] = { "+", "-", "*", "/", "nCr", "nPr", "ncr", "npr", "^",
				"~", "√", "sqrt", "ⁿ√", "n√", "!", "%", ")", "(", "x²", "sin",
				"cos", "tan", "arcsin", "arccos", "arctan", "log", "STO", "sto",
				"mod", "and", "or", "xor", "not", "&", "âˆ¨", "â�»", "Â¬", "<<",
				">>", "â‰«", "â‰ª" };
	...
	private int priority(String s) {
		...
		p++;
		if (s.equals("and") || s.equals("&") || s.equals("or") || s.equals("âˆ¨")
				|| s.equals("xor") || s.equals("â�»") || s.equals("mod")
				|| s.equals(">>") || s.equals("<<") || s.equals("â‰«")
				|| s.equals("â‰ª")) {
			return p;
		}
		p++;
		if (s.equals("nCr") || s.equals("nPr") || s.equals("ncr")
				|| s.equals("npr")) {
			return p;
		}
		p++;
		if (s.equals("not") || s.equals("Â¬")) {
			return p;
		}
		p++;
		if (s.equals("~")) {
		...
		if (s.equals("√") || s.equals("n√") || s.equals("ⁿ√") || s.equals("!")
				|| s.equals("^") || s.equals("x²") || s.equals("sqrt")) {
			return p;
		}
		p++;
		return 0;
	...
	// // chuan hoa bieu thuc toan hoc, them cac phĂ©p toan can thiet
	...
	// s1 = s1 + "~ ";
	// }// VD hoac 6Ï€ , ...)Ï€ chuyen sang 6*Ï€ , ...)*Ï€
	// else if (i > 0
	...
	public Double valueMath(String math) {
		...
		for (int i = 0; i < elementMath.length; i++) {
			...
			if (!isOperator(elementMath[i])) {
				...
				if (ei.equals("~")) {
				...
				} else if (ei.equals("not") || ei.equals("Â¬") || ei.equals("!")) {
					if (isIntegerNumber(num1) && num1 >= 0) {
						if (ei.equals("not") || ei.equals("Â¬")) {
							num = ~(long) num1;
						} else if (ei.equals("!")) {
					...
					if (ei.equals("STO") || ei.equals("sto")) {
						...
						if (ei.equals("ncr") || ei.equals("nCr")) {
							num = combination((int) num2, (int) num1);
							S.pop();
						} else if (ei.equals("npr") || ei.equals("nPr")) {
							num = permutation((int) num2, (int) num1);
							S.pop();
						} else if (ei.equals("and") || ei.equals("&")) {
							num = (long) num2 & (long) num1;
							S.pop();
						} else if (ei.equals("or") || ei.equals("âˆ¨")) {
							num = (long) num2 | (long) num1;
							S.pop();
						} else if (ei.equals("xor") || ei.equals("â�»")) {
							num = (long) num2 ^ (long) num1;
							S.pop();
						} else if (ei.equals("mod")) {
							num = (long) num2 % (long) num1;
							S.pop();
						} else if (ei.equals("<<") || ei.equals("â‰ª")) {
							num = (long) num2 << (long) num1;
							S.pop();
						} else if (ei.equals(">>") || ei.equals("â‰«")) {
